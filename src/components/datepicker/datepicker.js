import {
  getMonthAllDays, getAboutYears, getAllMonth
} from './time';
...
export default {
  ...
  props: { value: { type: [Number, String, Object] } },
  ...
  methods: {
    ...
    renderMonths() {
      const months = getAllMonth(this.M);

      return l3.map((l, index) => <tr>
        {months.slice(index * 4, (index + 1) * 4).map(val => <td onClick={() => this.handlerMonth(index)}>{val}</td>)}
      </tr>);
    },

    renderYears() {
      const years = getAboutYears(this.Y);

      return l3.map((l, index) => <tr>
        {years.slice(index * 4, (index + 1) * 4).map(val => <td onClick={() => this.handlerYears(val)}>{val}</td>)}
      </tr>);
    },
