...
export const getMonthAllDays = function(year, month) {
  ...
  // 获得当月天数
  let curMonthDays = getConutDay(year, month, date)

  // 当月首尾日
  let lastDay = date.getDay()
  let fristDay = getFirstDay(date)
  // 获取上个月天数
  let lastYear = month === 1 ? year - 1 : year
  let lastMonth = month === 1 ? 12 : month - 1
  let lastMonthDays = getConutDay(lastYear, lastMonth, date)

  let result = []
  // 日期前补
  for (let i = fristDay; i > 0; i--) {
    let day = lastMonthDays - i + 1
    result.push({ day, month: 'prev' })
  }

  // 遍历
  for (let i = 1; i < curMonthDays + 1; i++) {
    result.push({ day: i, month: 'now' })
  }
  ...
  if (result.length === 35) {
    for (let i = 1; i <= 7; i++) {
      let day = result[34].day > 27 ? i : result[34].day + i
      result.push({ day, month: 'next' })
    }
  }
